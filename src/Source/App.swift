            ...
            if let constantsDictionary = value as? Dictionary<String, Any> {
                ...
                for (key, value) in Array(constantsDictionary).sorted(by: {$0.0 < $1.0}) {
                    ...
                    if ((value as AnyObject).className.contains("Dictionary")) {
                        lineValue = writeConstants(key, value: value, type: type, level: level + 1)
                    } else {
            ...
            if let constantsDictionary = value as? Dictionary<String, Any> {
                for (key, value) in Array(constantsDictionary).sorted(by: {$0.0 < $1.0}) {
                    ...
                    if ((value as AnyObject).className.contains("Dictionary")) {
                        ...
                        case .CustomEnum:
                            let line = "public static final " + lineValue.results.enumType.snakeCaseToCamelCase() + " " + key.uppercased() + " = \(lineValue.output);\n"
                        ...
                        default:
                            ...
                            let line = tabs + "\tpublic static final " + lineValue.type.rawValue + " " + key.uppercased() + " = \(lineValue.output);\n"
                            outputClassString.append(line)
                            skipClass = false
                        }
                    }
